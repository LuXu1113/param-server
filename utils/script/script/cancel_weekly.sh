...
# generate task id #
TASK_ID=cancel_weekly_$(hostname)_$(date +%Y%m%d%H%M%S)_$$
...
for log in ${all_jobs}; do
  ...
  job_torque_id=`cat ${LOG_DIR}/${log}/weekly_trainer.sh.log | grep "mpi_job_id:" | tail -n 1 | awk '{ print $NF }' | awk 'BEGIN { FS = "." } { print $1 }'`
