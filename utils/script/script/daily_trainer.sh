...
tmp=`is_valid_hdfs_path ${conf_hdfs_path_of_weekly_model}`
is_valid_input_path=`echo ${tmp} | awk 'BEGIN { RS = "," } { print $0 }' | grep "is_valid" | awk 'BEGIN { FS = "=" } { print $2 }'`
if [[ ${is_valid_input_path} -eq 0 ]]; then
  message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][FATAL] invalid hdfs path:\
           conf_hdfs_path_of_weekly_model=${conf_hdfs_path_of_weekly_model}, valid format is: hdfs://[name_node]:[port]/..."
  echo ${message} | tee -a ${SCRIPT_LOG}
  exit 3
fi
...
tmp_str_1=`${HADOOP_FS_CMD} -text ${conf_hdfs_path_of_weekly_model}/donefile | grep "${conf_hdfs_path_of_weekly_model}" | tail -n 1`
if [[ -z ${tmp_str_1} ]]; then
  message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][FATAL] fail to read weekly model donefile:\
           ${conf_hdfs_path_of_weekly_model}/donefile."
  echo ${message} | tee -a ${SCRIPT_LOG}
  exit 3
fi
...
recovery_mode=0
tmp_str_2=`${HADOOP_FS_CMD} -text ${conf_hdfs_path_of_output_model}/donefile | grep "${conf_hdfs_path_of_weekly_model}" | tail -n 1`
if [[ ${tmp_str_1} != ${tmp_str_2} ]]; then
  ${HADOOP_FS_CMD} -text ${conf_hdfs_path_of_output_model}/donefile > ${local_working_dir}/donefile
  ${HADOOP_FS_CMD} -text ${conf_hdfs_path_of_weekly_model}/donefile | grep "${conf_hdfs_path_of_weekly_model}" | tail -n 1 >> ${local_working_dir}/donefile
...
if [[ ${tmp_str_1} != ${tmp_str_2} ]]; then
  message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][FATAL] fail to merge donefile:\
           ${conf_hdfs_path_of_weekly_model}/donefile -> ${conf_hdfs_path_of_output_model}/donefile."
  echo ${message} | tee -a ${SCRIPT_LOG}
  exit 3
fi
...
if [[ ${recovery_mode} -eq 0 ]]; then
  tmp_str=`${HADOOP_FS_CMD} -text ${conf_hdfs_path_of_output_model}/donefile | grep ${conf_hdfs_path_of_weekly_model} | tail -n 1`
else
...
begin_date_of_training_data=$(date +%Y%m%d -d "1 day $(basename ${prior_model_path})")
if [[ $? -ne 0 ]]; then
  message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][FATAL] fail to get begin_date_of_training_data:\
...
# get number of days of training data
curr=${end_date_of_training_data}
number_of_days_of_training_data=0
while [[ ${curr} -ge ${begin_date_of_training_data} ]]; do
  curr=$(date -d "1 day ago ${curr}" +%Y%m%d)
  number_of_days_of_training_data=$((${number_of_days_of_training_data} + 1))
done

# set enviroments
sh ${SCRIPT_DIR}/common/modify_conf_batch.sh \
  "${end_date_of_training_data}" \
  "${number_of_days_of_training_data}" \
...
if [[ ${ret} -eq 0 ]]; then
  ...
  while [ ${max_unknown_status} -gt 0 ]; do
    ...
    if [[ "${status}" == "Q" ]]; then
      ...
      for node in ${nodes_list}; do
        ...
        if [[ ${has_server} != "true" || ${has_worker} != "true" ]]; then
          message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][WARNING] package not ready on ${node}, restart after 1 minutes."
          echo ${message} | tee -a ${SCRIPT_LOG}
          sleep 1m
        ...
        if [[ ${is_server_running} != "true" || ${is_worker_running} != "true" ]]; then
          message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][WARNING] start process fail on ${node}, restart."
          echo ${message} | tee -a ${SCRIPT_LOG}
          qdel `echo ${mpi_job_id} | awk 'BEGIN { FS = "." } { print $1 }'`
          break
        fi
    ...
    if [[ ${hdfs_dst_dir} != "" ]]; then
      ...
      tmp_local_first_node_file=${local_logging_dir}/first_node_$$.log
      tmp_dst_hdfs_file=${hdfs_dst_dir}/running_first_node_$$.log
...
last_model=`${HADOOP_FS_CMD} -text ${conf_hdfs_path_of_output_model}/donefile | tail -n 1 | awk '{ print $1 }'`
...
if [[ "${last_model_date}" == "${end_date_of_training_data}" ]]; then
  ...
  message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][INFO] training fail."
  echo ${message} | tee -a ${SCRIPT_LOG}
  exit 8
fi
