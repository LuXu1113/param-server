...
#-----------------------------------  launch  ---------------------------------------------#
curl http://release.sm.alibaba-inc.com/api2/data/release?name=${conf_model_name}\&token=${conf_token}\&users=${conf_alarm_user}\&src=${src}
if [[ $? -ne 0 ]]; then
  message="$(date +"%Y/%m/%d %H:%M:%S")][${SCRIPT_NAME}:$LINENO][FATAL] \
